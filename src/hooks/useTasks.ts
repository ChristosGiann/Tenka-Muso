...
export function useTasks(firebaseUser: User | null) {
    ...
    async function toggleDone(taskId: string, occurrenceDate?: string) {
        if (!firebaseUser) return;
        ...
        if (task.type === "routine" && occurrenceDate) {
            const currentStatus = task.routineCompletions?.[occurrenceDate] ?? "pending";
            const nextStatus = currentStatus === "done" ? "pending" : "done";

            await updateDoc(taskRef, {
                [`routineCompletions.${occurrenceDate}`]:
                    nextStatus === "done" ? "done" : deleteField(),
                updatedAt: serverTimestamp(),
            });
