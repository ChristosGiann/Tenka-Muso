...
import type { GoalProgressItem } from "../utils/goals";
...
export function GoalsPanel({
    ...
    const progressByGoalId = new Map(
        goalProgressItems.map((progressItem) => [
            progressItem.goal.id,
            progressItem,
        ])
    );
    return (
                ...
                <div className="grid gap-3 md:grid-cols-2">
                    {goals.map((goal) => (
                        <div key={goal.id} className={`${theme.innerPanel} p-4`}>
                            ...
                            {progressByGoalId.get(goal.id) && (
                                <div className="mt-4">
                                    <div className="mb-2 flex items-center justify-between gap-3 text-sm font-bold text-neutral-700">
                                        <span>
                                            {progressByGoalId.get(goal.id)?.formattedActual} /{" "}
                                            {progressByGoalId.get(goal.id)?.formattedTarget}
                                        </span>

                                        <span>{progressByGoalId.get(goal.id)?.percentage}%</span>
                                    </div>
                                    ...
                                    <div className="h-3 overflow-hidden rounded-full border border-neutral-300 bg-stone-200">
                                        <div
                                            ...
                                            style={{
                                                width: `${progressByGoalId.get(goal.id)?.percentage ?? 0}%`,
                                            }}
                                        />
                                    </div>

                                    <p className="mt-2 text-xs font-semibold text-neutral-500">
                                        {progressByGoalId.get(goal.id)?.periodLabel}:{" "}
                                        {progressByGoalId.get(goal.id)?.periodStart} →{" "}
                                        {progressByGoalId.get(goal.id)?.periodEnd}
                                    </p>

                                    <p className="mt-1 text-xs font-semibold text-neutral-500">
                                        Remaining: {progressByGoalId.get(goal.id)?.formattedRemaining}
                                    </p>
                                </div>
                            )}
